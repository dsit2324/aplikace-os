...
## Additional: Run minimal Flask + Postgres service

This repository now includes a minimal Flask app using SQLAlchemy in `app/` and a `web` + `db` service in the top-level `docker-compose.yml` for local testing.

Quick start for Flask + Postgres:
...
API endpoints:
- GET /items
- POST /items  (JSON body: {"name": "my item"})
